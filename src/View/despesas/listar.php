...
<div class="cad-tabs">
    ...
    <a href="<?= BASE_URL ?>?rota=despesas"      class="cad-tab active"><i class="fa fa-money"></i> Despesas</a>
    <a href="<?= BASE_URL ?>?rota=usuarios"      class="cad-tab"><i class="fa fa-users"></i> Usuários</a>
</div>

<!-- Formulário Nova Despesa -->
<div class="cad-form-card">
    <div class="cad-form-title"><i class="fa fa-plus-circle" style="color:#005b96;"></i> Nova Despesa</div>
    <form action="<?= BASE_URL ?>?rota=despesas.salvar" method="POST">
        <div class="cad-form-grid">
            <div class="cad-span-6">
                <label class="cad-field-label">Descrição</label>
                <input type="text" name="descricao" class="cad-field-input" required>
            </div>
            <div class="cad-span-3">
                <label class="cad-field-label">Valor (R$)</label>
                <input type="number" step="0.01" name="valor" class="cad-field-input" required>
            </div>
            <div class="cad-span-3">
            ...
            <div class="cad-span-3">
                <label class="cad-field-label">Data</label>
                <input type="date" name="data_despesa" class="cad-field-input" required>
            </div>
        </div>
        <button type="submit" class="cad-btn-save"><i class="fa fa-check"></i> Salvar Despesa</button>
    </form>
</div>
...
<div class="cad-card">
    <div class="cad-card-header">
        ...
        <div class="cad-busca-wrap">
            ...
            <input type="text" id="busca_despesas" placeholder="Buscar despesa..." autocomplete="off">
        </div>
    </div>

    <table class="cad-table mobile-card-table" id="tabela_despesas">
        <thead>
            <tr>
                <th>Data</th>
                <th>Descrição</th>
        ...
        <tbody>
            <?php if (count($despesas) > 0): ?>
                ...
                <tr>
                    <td style="color:#64748b;font-size:0.82rem;white-space:nowrap;"><?= date('d/m/Y', strtotime($d['data_despesa'])) ?></td>
                    <td style="font-weight:600;color:#0f172a;"><?= htmlspecialchars($d['descricao']) ?></td>
                    <td style="font-weight:700;color:#ef4444;">R$ <?= number_format($d['valor'], 2, ',', '.') ?></td>
                    <td>
                        <?php if ($d['tipo'] === 'fixa'): ?>
                            <span class="cad-badge" style="background:#fef3c7;color:#b45309;">Fixa</span>
                        <?php else: ?>
                            <span class="cad-badge" style="background:#f0fdf4;color:#16a34a;">Variável</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <div style="display:flex;gap:6px;justify-content:flex-end;">
                            <a href="<?= BASE_URL ?>?rota=despesas.excluir&id=<?= $d['id'] ?>" class="cad-btn-del" title="Remover" onclick="return confirm('Remover esta despesa?');"><i class="fa fa-trash"></i></a>
                        </div>
                    </td>
                </tr>
                <?php endforeach; ?>
            ...
            <tr><td colspan="5" style="text-align:center;padding:40px;color:#94a3b8;">
                <i class="fa fa-money" style="display:block;font-size:2rem;margin-bottom:8px;color:#cbd5e1;"></i>
...
<script>
(function(){
    var inp = document.getElementById('busca_despesas');
    var tbl = document.getElementById('tabela_despesas');
    if (!inp||!tbl) return;
    var _t = null;
