...
<div class="rel-tabs">
    <a href="<?= BASE_URL ?>?rota=relatorios.diario"       class="rel-tab active"><i class="fa fa-calendar"></i> Relatório do Dia</a>
    <?php if (is_admin()): ?>
...
<!-- Navegação de data -->
<div class="rel-card">
    <div class="rel-dia-nav">
        <a href="<?= BASE_URL ?>?rota=relatorios.diario&data=<?= $data_anterior ?>" class="rel-dia-btn">&#8249; Dia Anterior</a>
        <div>
            <div class="rel-dia-titulo">Relatório do Dia: <?= date('d/m/Y', strtotime($data_selecionada)) ?></div>
        </div>
        <a href="<?= BASE_URL ?>?rota=relatorios.diario&data=<?= $data_posterior ?>" class="rel-dia-btn">Próximo Dia &#8250;</a>
    </div>
    <form method="GET" action="<?= BASE_URL ?>index.php" style="text-align:center;">
        <input type="hidden" name="rota" value="relatorios.diario">
        <div>
            <label class="rel-field-label">Selecionar outra data</label>
            <input type="date" name="data" class="rel-field-input" value="<?= $data_selecionada ?>" onchange="this.form.submit()">
        </div>
    </form>
</div>
...
<div class="rel-stats-grid">
    <div class="rel-stat azul">
        <div class="rel-stat-label">Entrada Bruta</div>
        <div class="rel-stat-value">R$ <?= number_format($faturamento_bruto, 2, ',', '.') ?></div>
    </div>
    <div class="rel-stat vermelho">
        <div class="rel-stat-label">Taxas de Cartão</div>
        <div class="rel-stat-value vermelho">- R$ <?= number_format($total_taxas, 2, ',', '.') ?></div>
    </div>
    <div class="rel-stat vermelho">
        <div class="rel-stat-label">Custo Auxiliar</div>
        <div class="rel-stat-value vermelho">- R$ <?= number_format($total_custo_auxiliar, 2, ',', '.') ?></div>
    </div>
    <div class="rel-stat vermelho">
        <div class="rel-stat-label">Saídas (Despesas)</div>
        <div class="rel-stat-value vermelho">- R$ <?= number_format($total_despesas, 2, ',', '.') ?></div>
    </div>
    <div class="rel-stat verde">
        <div class="rel-stat-label">Lucro Líquido do Dia</div>
        <div class="rel-stat-value verde">R$ <?= number_format($lucro_liquido, 2, ',', '.') ?></div>
    </div>
</div>
<?php endif; ?>
...
<div class="rel-card">
    <div class="rel-card-title"><i class="fa fa-user-md" style="color:#005b96;"></i> Pagamentos por Dentista</div>
    <div class="rel-table-wrap">
        <table class="rel-table">
            <thead><tr><th>Dentista</th><th style="text-align:right;">Valor a Pagar</th></tr></thead>
            <tbody>
                <?php if (count($pagamento_dentistas) > 0): ?>
                    <?php foreach ($pagamento_dentistas as $d): ?>
                    <tr>
                        <td style="font-weight:600;"><?= htmlspecialchars($d['nome']) ?></td>
                        <td style="text-align:right; font-weight:700; color:#00b894;">R$ <?= number_format($d['total_comissao'], 2, ',', '.') ?></td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                <tr><td colspan="2" style="text-align:center; padding:32px; color:#94a3b8;">Nenhum atendimento comissionado no dia.</td></tr>
                <?php endif; ?>
            </tbody>
            <?php if (is_admin()): ?>
            <tfoot>
                <tr>
                    ...
                    <td style="text-align:right; color:#e74c3c;">- R$ <?= number_format($total_comissoes, 2, ',', '.') ?></td>
                </tr>
            </tfoot>
            <?php endif; ?>
        </table>
...
<!-- Despesas do dia -->
<div class="rel-card">
    <div class="rel-card-title"><i class="fa fa-money" style="color:#e74c3c;"></i> Despesas Detalhadas do Dia</div>
    <div class="rel-table-wrap">
        <table class="rel-table">
            <thead><tr><th>Descrição</th><th>Tipo</th><th style="text-align:right;">Valor</th></tr></thead>
            <tbody>
                <?php if (count($despesas_dia) > 0): ?>
                    <?php foreach ($despesas_dia as $d): ?>
                    <tr>
                        <td><?= htmlspecialchars($d['descricao']) ?></td>
                        <td><span style="background:#f1f5f9;color:#64748b;padding:2px 8px;border-radius:20px;font-size:0.72rem;font-weight:700;"><?= ucfirst($d['tipo']) ?></span></td>
                        <td style="text-align:right; font-weight:600;">R$ <?= number_format($d['valor'], 2, ',', '.') ?></td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                <tr><td colspan="3" style="text-align:center; padding:32px; color:#94a3b8;">Nenhuma despesa registrada para este dia.</td></tr>
                <?php endif; ?>
            </tbody>
            <tfoot>
                <tr>
                    <td colspan="2">Total Despesas</td>
                    <td style="text-align:right;">R$ <?= number_format($total_despesas, 2, ',', '.') ?></td>
                </tr>
            </tfoot>
        </table>
    </div>
