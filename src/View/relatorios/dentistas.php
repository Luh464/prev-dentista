...
<div class="rel-tabs">
    ...
    <a href="<?= BASE_URL ?>?rota=relatorios.dentistas"     class="rel-tab active"><i class="fa fa-user-md"></i> Por Dentista</a>
...
<div class="rel-card">
    <form method="GET" action="<?= BASE_URL ?>index.php">
        ...
        <div class="rel-filter-row">
            ...
            <?php if (is_admin()): ?>
            <div>
                <label class="rel-field-label">Dentista</label>
                <select name="dentista_id" class="rel-field-input">
                    <option value="todos">Todos</option>
                    <?php foreach ($dentistas as $d): ?>
                    <option value="<?= $d['id'] ?>" <?= $dentista_id == $d['id'] ? 'selected' : '' ?>><?= htmlspecialchars($d['nome']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <?php endif; ?>
...
<div class="rel-card">
    <div class="rel-card-title"><i class="fa fa-user-md" style="color:#005b96;"></i> Desempenho por Dentista</div>
        ...
        <table class="rel-table">
            ...
            <tbody>
                <?php if (count($relatorio_dentistas) > 0): ?>
                    ...
                    <tr>
                        <td style="font-weight:600; color:#005b96;"><?= htmlspecialchars($rel['dentista_nome']) ?></td>
                        <td><?= $rel['total_atendimentos'] ?></td>
                        <td>R$ <?= number_format($rel['faturamento_bruto'], 2, ',', '.') ?></td>
                        <td style="font-weight:700; color:#00b894;">R$ <?= number_format($rel['valor_para_dentista'], 2, ',', '.') ?></td>
                        <td style="text-align:right; font-weight:700; color:#00b894;">R$ <?= number_format($rel['valor_para_clinica'], 2, ',', '.') ?></td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                <tr><td colspan="5" style="text-align:center; padding:32px; color:#94a3b8;">Nenhum dado encontrado para os filtros selecionados.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
